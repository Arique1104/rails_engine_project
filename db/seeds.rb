...
CSV.foreach(Rails.root.join('db/data/merchants.csv'), headers: true) do |row|
  ...
  hashed_row = row.to_h
  Merchant.create!(name: hashed_row["name"],
                   created_at: hashed_row["created_at"],
                   updated_at: hashed_row["updated_at"])
end

CSV.foreach(Rails.root.join('db/data/customers.csv'), headers: true) do |row|
  hashed_row = row.to_h
  Customer.create!(first_name: hashed_row["first_name"],
                   last_name: hashed_row["last_name"],
                   created_at: hashed_row["created_at"],
                   updated_at: hashed_row["updated_at"])
end
...
CSV.foreach(Rails.root.join('db/data/items.csv'), headers: true) do |row|
  ...
  Item.create!(name: hashed_row["name"],
               ...
               merchant_id: hashed_row["merchant_id"],
               created_at: hashed_row["created_at"],
               updated_at: hashed_row["updated_at"])
end
...
CSV.foreach(Rails.root.join('db/data/invoices.csv'), headers:true) do |row|
  hashed_row = row.to_h
  Invoice.create!(status: hashed_row["status"],
                  customer_id: hashed_row["customer_id"],
                  merchant_id: hashed_row["merchant_id"],
                  created_at: hashed_row["created_at"],
                  updated_at: hashed_row["updated_at"])
end
...
CSV.foreach(Rails.root.join('db/data/transactions.csv'), headers: true) do |row|
  hashed_row = row.to_h
  Transaction.create!(credit_card_number: hashed_row["credit_card_number"],
                      result: hashed_row["result"],
                      created_at: hashed_row["created_at"],
                      updated_at: hashed_row["updated_at"],
                      invoice_id: hashed_row["invoice_id"])
end

CSV.foreach(Rails.root.join('db/data/invoice_items.csv'), headers: true) do |row|
  hashed_row = row.to_h
  InvoiceItem.create!(quantity: hashed_row["quantity"],
                      unit_price: hashed_row["unit_price"],
                      created_at: hashed_row["created_at"],
                      updated_at: hashed_row["updated_at"],
                      item_id: hashed_row["item_id"],
                      invoice_id: hashed_row["invoice_id"])
end
